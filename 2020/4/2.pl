...
sub validate_hgt {
   my ($height) = @_;
   if (index($height, 'cm') > -1) {
      my $cm = substr($height, 0,3);
      return $cm >= 150 && $cm <= 193;
   }
   if (index($height,'in') > -1) {
      my $cm = substr($height, 0,2);
      return $cm >= 59 && $cm <= 76;
   }
   return 0;
}
...
sub validate_ecl {
   my $ecl = shift;
   my @valid = qw/amb blu brn gry grn hzl oth/;
   return grep { $ecl eq $_ } @valid;
}
...
chomp($line);
    if (!$line) {
        if ( validate_int(1920,2002,$cur{'byr'}) &&
             validate_int(2010,2020,$cur{iyr}) &&
             validate_int(2020,2030,$cur{eyr}) &&
             validate_hgt($cur{hgt}) &&
             validate_hcl($cur{hcl}) &&
             validate_ecl($cur{ecl}) &&
             validate_pid($cur{pid})) {
            $valid++
        }
